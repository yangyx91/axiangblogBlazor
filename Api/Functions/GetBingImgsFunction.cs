...
namespace Api.Functions
{
    ...
    {
        private static readonly string _dbName = "axiangdb";
            ...
            [HttpTrigger(AuthorizationLevel.Function, "get", "post", Route = "allbingImgs")] HttpRequest req)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                var response = await client.GetAsync(_dbName + "/_all_docs?include_docs=true&limit=50");
                if (response.IsSuccessStatusCode)
                {
                    var responseJson = await response.Content.ReadAsStringAsync();
                    await new ApiLogger("GetBingImgsFunction").LogInformation(responseJson, new
                    {
                        BaseAddress = client.BaseAddress,
                        RequestUri = _dbName + "/_all_docs?include_docs=true&limit=50"
                    });
                    return new OkObjectResult(System.Text.Json.JsonSerializer.Deserialize<QueryBingImgDocumentResult>(responseJson));
                }
                else
                {
                    string msg = "Failure to Get. Status Code: " + response.StatusCode + ". Reason: " + response.ReasonPhrase;
                    await new ApiLogger("GetBingImgsFunction").LogError(msg, new
                    {
                        BaseAddress = client.BaseAddress,
                        RequestUri = _dbName + "/_all_docs?include_docs=true&limit=50"
                    });
